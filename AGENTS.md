...
Never cut: input validation at trust boundaries, error handling where data can be lost, auth/secrets/permissions/injection defenses, or UI accessibility. Scope cuts never touch this floor. If the codebase has tests, the change gets one. If it has none, say so once and move on.
